...
/*   Updated: 2025/08/20 01:42:13 by shirakim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void handle_dq(t_lexer_handler *handler, char **s, t_shell *shell)
{
    (*s)++;
    while (**s && **s != '\"')
    {
...
static void handle_nq(t_lexer_handler *handler, char **s, t_shell *shell)
{
    if (**s == '$' && ft_isalpha((*s)[1]))
        handle_var(handler, s, shell);
    else
...
{
    (void)shell; // temporal si aún no lo usas dentro de get_cmds_info
    cmd_line->line = ft_strdup(line);
    cmd_line->execute = true;

    // Pasamos shell si tu get_cmds_info necesita información del shell
    get_cmds_info(cmd_line, shell, line);

    // if (cmd_line->execute)
    //     print_info(cmd_line);
    // free_cmd_line(cmd_line); // no liberar aquí, se hace al final de exec_line
}
