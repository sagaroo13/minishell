...
/*   Updated: 2025/08/20 00:16:40 by shirakim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_cmds_info(t_command_line *cmd_line, t_shell *shell, char *line)
{
    ...
    i = -1;
    while (line_parts[++i])
        get_cmd_info(cmd_line, &cmd_line->cmds[i], line_parts[i], shell);  // <-- PASAMOS shell
...
void push_buffer(t_lexer_handler *handler, bool quoted)
{
    ...
    handler->tokens[handler->argc].quoted = quoted ? true : false;
